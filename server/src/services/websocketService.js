...
function handleConnectionTest(clientId) {
  ...
  performRandomLongOperation(clientId)
    .then(result => {
      ...
      console.log(`客户端 ${clientId} 的长时间操作成功完成，耗时: ${result.totalTime}ms`);
    })
    .catch(error => {
      // 发送操作失败消息
      sendMessage(clientId, {
...
async function performRandomLongOperation(clientId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 如果会失败，随机决定在哪一步失败
    const failAt = willFail ? Math.floor(Math.random() * (totalSteps - 1)) + 1 : -1;
    ...
    const executeStep = () => {
      ...
      const issue = hasIssue ? randomIssues[Math.floor(Math.random() * randomIssues.length)] : null;
      
      // 构建进度消息
      let progressMessage = `${operationType}: ${phases[currentStep % phases.length]} (${progress}%)`;
      if (hasIssue) {
      ...
      console.log(`客户端 ${clientId} 的操作进度: ${progress}%, 步骤 ${currentStep}/${totalSteps}, 耗时 ${stepTime}ms`);
      ...
      if (currentStep >= totalSteps) {
        setTimeout(() => {
          ...
          // 计算平均每步耗时
          const averageStepTime = Math.round(stepTimes.reduce((a, b) => a + b, 0) / stepTimes.length);
